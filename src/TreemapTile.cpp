                    ...
                    Orientation    orientation )
{
    ...
    IFileInfoSortedBySizeIterator it( _orig, minSize );
...
void TreemapTile::createSquarifiedChildren( const QRectF & rect )
{
    ...
    while ( *it )
    {
    IFileInfoList row = squarify( childrenRect, scale, it );
    childrenRect = layoutRow( childrenRect, scale, row );
    }
}


IFileInfoList TreemapTile::squarify( const QRectF & rect,
                    double	  scale,
                    IFileInfoSortedBySizeIterator & it )
{
                   ...
                   IFileInfoList & row )
{
    ...
    IFileInfoList::const_iterator it  = row.constBegin();
    IFileInfoList::const_iterator end = row.constEnd();
             ...
             QWidget			* widget )
{
    ...
    else
    {
        ...
        // tile (i.e., if the corresponding IFileInfo doesn't have any
...
void TreemapTile::contextMenuEvent( QGraphicsSceneContextMenuEvent * event )
{
    ...
    IFileInfoSet selectedItems = _parentView->selectionModel()->selectedItems();
