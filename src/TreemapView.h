...
namespace QDirStat
{
     ...
     * IFileInfo node. Returns 0 if there is none.
     *
     * Notice: This is an expensive operation since all treemap tiles need
     * to be searched.
     **/
    TreemapTile * findTile( const IFileInfo * node );
     ...
     **/
    QColor tileColor( IFileInfo * file );
     ...
     **/
    void updateSelection( const IFileInfoSet & newSelection );

    /**
     * Update the current item that has been changed in another view.
     **/
    void updateCurrentItem( IFileInfo * currentItem );
    ...
    /**
     * Search the treemap for a tile with the specified IFileInfo node and
     ...
     * current item.
     **/
    void setCurrentItem( IFileInfo * node );
    ...
    /**
     * Zoom out one level: The parent (if there is any) IFileInfo node of
     ...
     **/
    void deleteNotify( IFileInfo * node );
    ...
    void sendSelection();
         ...
         **/
        void sendHoverEnter( IFileInfo * node );

        /**
         * Send a hoverLeave() signal for 'node'.
         **/
        void sendHoverLeave( IFileInfo * node );
     ...
     **/
    void rebuildTreemap( IFileInfo *	    newRoot,
     ...
     **/
    void scheduleRebuildTreemap( IFileInfo * newRoot );
     ...
     **/
    void selectionChanged( IFileInfo * item );

    /**
     * Emitted when the current item changes.
     **/
    void currentItemChanged( IFileInfo * newCurrent );
    ...
    void contextMenu( TreemapTile * tile, const QPoint & pos );
         ...
         **/
        void hoverEnter( IFileInfo * item );

        /**
         * Emitted when the mouse cursor leaves the hover over 'item'.
         **/
        void hoverLeave( IFileInfo * item );
